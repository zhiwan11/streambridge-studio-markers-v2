...
const server = app.listen(0, async () => {
  try {
    ...
    for (const name of ["60 FPS", "120 FPS", "HQ High Bitrate", "HDR Vivid", "FLAC"]) {
      const filter = byName.get(name);
      if (!filter) throw new Error(`missing ${name}`);
      if (!filter.imageURL.includes("?v=3.0.4")) throw new Error(`stale asset version: ${name}`);
    }

    const cases = [
      ["Movie.2160p.60fps.HQ.HDRVivid.FLAC", ["60 FPS", "HQ High Bitrate", "HDR Vivid", "FLAC"]],
      ["Movie.2160p.119.88Hz.HDR-Vivid", ["120 FPS", "HDR Vivid"]],
      ["Movie.1960.WEB-DL.HQMUX.HDR10", []],
    ];
    for (const [value, expected] of cases) {
      const hits = ["60 FPS", "120 FPS", "HQ High Bitrate", "HDR Vivid", "FLAC"]
